...
## 2026-05-27 14:35 — #1608 v2 PLAN-KILL 3/3

- **Timestamp**: 2026-05-27 14:35 UTC
  - **Action**: v2 plan PLAN-KILLED 3/3 (Codex r2 task-mpokuixd-yvxfw0 + AGY r2 adversarial-review-mpokum1e-f1gn74 + Claude SMR r3 self-correction). 10 new fatals on v2: (F-V2-1) rate-deny hits policy-Deny path = 1Mpps event-emit DoS; (F-V2-2) :2393 deny doesn't drop, continues to session install; (F-V2-3) eviction-storm grants full burst on every miss; (F-V2-4) bucket key omits zone/profile; (F-V2-5) skipping policy_id==0 = 0% cache hit on default-deny floods; (F-V2-6) const_assert values wrong (TokenBucket=32 not 24, VerdictCacheEntry=96 not 88) — won't compile; (F-V2-7) per-binding state vs per-worker budget claim mismatch; (F-V2-8) rule_idx not in PolicyEvaluationResult, hit_counter.add private; (F-V2-9) mem::size_of<PolicyRule> assert misses semantic field additions; (F-V2-10) helper signature omits now_ns. Per /engineer protocol second PLAN-KILL on different design escalates to user; no v3 without authorization. Branch preserved at 7e6a596ca + claude-smr-plan-r3 + PLAN-KILL-v2.
  - **File(s)**: docs/pr/1608-cold-path-hardening/claude-smr-plan-r3.md, docs/pr/1608-cold-path-hardening/PLAN-KILL-v2.md
