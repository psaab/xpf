...
- **Timestamp**: 2026-06-20
- **Action**: #1387 Inc-2 DDNS plan revised to r2 (companion-free hostile review). A hostile plan-review returned PLAN-NEEDS-MAJOR on r1's HA single-writer mechanism. Root finding (verified against source): r1's per-RG `subnet_id`-attribution single-writer gate is UNSOUND and UNNECESSARY. UNNECESSARY because the Kea config each node serves is rendered MASTER-FILTERED (daemon_apply.go:1095-1101 ApplyClusterCommit(d.filterDHCPConfigForMasterRGs(cfg)); daemon_ha.go:988-1041 keeps only currently-MASTER RG groups; dhcpserver.go:184/217/229 renders only handed-in groups into the per-family memfile) -> each node's memfile already contains ONLY its own MASTER-RG leases, disjoint by RG, so two nodes cannot publish the same record. UNSOUND because subnet_id is map-order-assigned (subnetID++ over a Go-map `Groups`, dhcpserver.go:511-518/588-595, types_system.go:902-903) -> per-node/per-render unstable, never a valid key (ddns_leases.go:63 already documents it as non-compared metadata). r2 corrections: (1) replaced the per-RG gate with a NODE-LEVEL gate ("reconcile my memfile IFF MASTER for >=1 RG", snapshotRethMasterState only) and DELETED the subnet_id map; documented the stable fallback (Address -> pool.Subnet -> group -> rethInterfacesForRG -> RG, daemon_ha.go:732/1235) as fallback-only, never subnet_id. (2) Added the async-takeover ordering note: MASTER-transition Kea reconcile is ApplyAsync (daemon_ha.go:925) so the DDNS nudge may fire before Kea repopulates the memfile -> benign (store-driven, add-only; zero deletes against a not-yet-populated memfile). (3) Resolved §11 Q1 (ship Domain-derived forward + canonical in-addr.arpa/ip6.arpa reverse; defer explicit forward-zone/reverse-zone leaves as additive follow-up; helper takes optional zone list) and §11 Q6 (PTR NOTAUTH/REFUSED = counted skip skipped_total{reason="ptr-notauth"}, not a blocking error, implemented now). Updated Prometheus reason enum (dropped not-owner-rg, added ptr-notauth), §9 test plan, R2 risk row. Bumped header to PLAN-READY (r2). Net effect: HA gate is SIMPLER than r1. Companion-free per directive (no Codex/AGY, no production source touched, no PR). Wrote claude-smr-plan-r2.md (the PLAN-NEEDS-MAJOR review that forced this revision).
- **File(s)**: docs/research/1387-inc2-ddns-backend/plan.md, docs/research/1387-inc2-ddns-backend/claude-smr-plan-r2.md, _Log.md
