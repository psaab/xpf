...
- **Timestamp**: 2026-06-19
- **Action**: #2008 H1 (`inactive:` statement) research plan DRAFT v1. Walked parser/lexer/AST/group-expansion/schema-walk/configstore-persistence to localize the silent-drop: `inactive:` lexes as one ident token (lexer treats `:` as ident char), parser lands it as `Keys[0]="inactive:"`, every compiler walk + schema walk keys off `Keys[0]` so the node is silently skipped (not compiled, not validated, no warning). Designed Path A (node `Inactive bool` flag stripped from Keys at parse + one centralized `stripInactiveNodes` pass in `compileConfigWithOpts` after the existing Clone, plus the same prune at the schema-validate entry) which keeps all 15 compiler files + the schema walk untouched. Key shipped-fact: persistence is `json.Marshal/Unmarshal` of the `Node` struct (db.go), so an `omitempty` field round-trips automatically — no DB/envelope change. Disposition: LIKELY-DEFER-MULTI-INCREMENT (inc1 = parse/persist/display/strip; inc2 = activate/deactivate CLI verbs). 7 open questions for adversarial review (strip-vs-ExpandGroups ordering, schema-validate-strip correctness, the pre-expand tunnel-id gate at compiler.go:163, CLI-verb scope, display fidelity).
- **File(s)**: docs/research/2008-vsrx-parity-tier2-h1-inactive/plan.md, _Log.md
