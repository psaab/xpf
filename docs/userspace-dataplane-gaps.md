...
| DataPlane control-plane contract | Userspace manager no longer embeds the legacy `dataplane.DataPlane`; a userspace `LegacyDataPlaneAdapter` owns old-interface compatibility while callers migrate. Operator metadata reads in API/gRPC/CLI/daemon now use `LastApplyResult()` instead of `LastCompileResult()`, with a canary preventing those surfaces from regressing to compile-result metadata. The manager still holds a named eBPF shim manager for XDP/map bootstrap state, and session/telemetry callers still need to move fully to domain interfaces; tracked by #1381 |
| Dataplane event logging | Session open/close/update are emitted by userspace. Policy-deny, screen-drop, and logged routing-instance filter hits now enqueue RT_FLOW frames through the non-blocking Rust event-stream producer with existing rate-limit/loss accounting and Go decode/status handling. Remaining #1379 evidence is end-to-end syslog validation plus broader filter-log coverage for non-PBR input/output/lo0 filter paths and richer policy/filter identity mapping. |
