...
- **Revision:** r2 (incorporates Claude-SMR-r1 hostile self-review)
- **Status:** PLAN-READY (recommended: Path B — explicit generated-reply
  classification contract, with §10 scope fenced to NOT touch port-mirror
  direction semantics)
...
### 6.2 Parse failure of the generated bytes

The bytes were just built by our own code, so a parse failure is a logic bug,
not an attacker input. Policy: **fail-OPEN to unclassified emit** (queue=None,
dscp=None, no drop) + bump a dedicated `generated_reply_classify_parse_errors`
counter. Rationale: silently dropping a control reply we just decided to send
because our own parser choked is worse than emitting it with default treatment;
the counter surfaces the bug. (This is the one place where fail-open beats
fail-closed — and it is gated by "we built these bytes", so the threat model is
internal-only.) Reviewers: challenge this — see SMR §A1.
...
| Parse fail-OPEN masks a builder bug | Low | Counter surfaces it; fail-open is gated on "we built these bytes" (§6.2); SMR challenges this |
