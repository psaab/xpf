...
- **Acceptance can't be a single-thread cargo bench** (v1 fatal #6 /
  ...
  counts; the #1607 harness is still TX-capped per #1615).
...
1. The cold-path bottleneck is **unmeasured** on supported hardware
   ...
   #1615 still caps the only flooder at 870 K pps.
...
lands (no saturating flooder). Until then, acceptance MUST be downgraded
to: (a) unit-level cycle counts via `core::arch` rdtsc microbench on the
gate primitive in isolation, AND (b) a smoke proof that the
established-flow hot path (v4+v6, push+reverse, all CoS classes) and
`make test-failover` ≤60 ms are unchanged. The flood-mitigation
acceptance criteria stay TBD pending #1615 resolution.
