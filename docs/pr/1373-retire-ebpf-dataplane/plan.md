...
| #1379 | Policy-deny, screen-drop, PBR filter logs, non-PBR input/output/lo0 filter logs, source-disambiguated FILTER_LOG syslog, and deterministic fanout coverage now emit from userspace; remaining work is live userspace-cluster syslog evidence if Phase 4 requires operator artifacts | Before Phase 4 |
