...
Slice 4 -- attested run:
  ...
  Before the attested run, verify `ping -e/-p` support on the peer/LAN refs
  (one `-c 1 -e <flow-id> -p <hex>` probe each); unsupported flags fail closed
  (traffic cell FAIL), blocking PASS without risking a false PASS.
...
## 7.2 Fold-3 implementation deviations and safety case
...
| Post-arm admission refusal classification | Once the successful arm/readiness branch selects marker rows, `ADMIT_STALE`, `ADMIT_FULL`, and `ADMIT_SHUTDOWN`, zero captured markers, or unreadable/mismatched Rust, ledger, metrics, or traffic evidence make the selected/admitted/completed join fail and emit FAIL. This is intentional: a refusal or missing measurement after the armed claim breaks it. Setup, fixture, and readiness unavailability before entering that branch remain VOID; once an arm RPC is attempted, arm failure is FAIL under the commitment principle (the attempt was made), matching the post-arm refusal mapping. |
| Independent marker identity/key | The D11 traffic commands use `ping -c 1 -e <flow-id> -p <selector>` for four lab-labeled probes. Those flow IDs are execution labels only; they are not treated as ledger join keys. The authoritative join remains `(node_id, request_id, permit_epoch, queue_epoch, queue_number)` plus origin and digest equality. The Go matcher accepts ICMP echo requests only (type 8/code 0), and the lab procedure must retain the `-e`/`-p` command output as traffic evidence. Adding ICMP identifier/sequence fields to the production ledger/Rust schema is deliberately out of scope for this lane. The selector is generated per run (16 random bytes, 32 lowercase hex); attribution rests on this per-run stimulus plus a quiesced cluster-locked lab and exact-4 count/digest accounting (consumed==4, digest-joined==4, suppressed==deny52==WouldPermit-count), not on per-packet probe identity. |
...
non-standalone, and equal to each final ledger `node_id`; no more than D11ManifestCap=32 internally selected rows
