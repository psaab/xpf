#!/usr/bin/env bash
#
# Initialise summary.md as a Phase A placeholder template. Used once
# during pre-stage scaffolding; Phase B overwrites the file via
# build-summary.sh which fills real values from artifacts/.
#
# Idempotent: if summary.md already exists with a non-template Phase B
# marker, this script refuses to overwrite. Pass --force to override.
...
if [[ -f "$OUT" ]] && ! grep -q 'Phase A draft' "$OUT" 2>/dev/null; then
    if [[ "${1:-}" != "--force" ]]; then
        echo "init-summary-template: refusing to overwrite existing non-template $OUT (pass --force)" >&2
        exit 1
    fi
fi
...
echo "init-summary-template: wrote $OUT"
