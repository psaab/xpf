...
**Overlap is now detected outside the screen (#9950):** the paragraph this replaces
recorded, deliberately, that no overlap detector existed — detecting a real teardrop
needs offset/length state across fragments, which the stateless screen cannot hold
(`fragment_assoc` ASSOCIATES a first fragment's verdict rather than RFC-reassembling).
#9950 adds that state as a dedicated dataplane control (`userspace-dp/src/fragment_overlap/`):
every fragment's byte range is checked against its datagram's prior ranges (receiver key +
routing domain) post-screen/pre-cache, and overlapping fragments are dropped fail-closed
(RFC 5722 tear-drop semantics), so enforcement never runs on bytes a later fragment
rewrites. The screen itself still does not reassemble and still skips TCP-flag checks on
non-first fragments; the ordering gap is closed by the drop, not by screen coverage.
...
| **Aggressive Session Aging** | `security flow aging early-ageout N; high-watermark N; low-watermark N` | Accelerate aging when the session table exceeds a configured watermark threshold | Medium | **Partially mitigated (#10890; configured aging remains #3440)** — per-worker userspace tables now pressure-shed at a fixed 90% of `max_sessions`: a validated TCP initial SYN sheds local handshake-incomplete TCP sessions (both directions where present) first; established sessions are never evicted for pressure, and admission still refuses if shedding cannot make room. The configured `early-ageout` / high-watermark / low-watermark values remain unwired and emit an accepted-only advisory. The Go-side GC watermark hysteresis remains skipped on the AF_XDP path. |
...
| Feature | Junos Config Path | Description | Priority | Status |
|---------|-------------------|-------------|----------|--------|
| **Policy-Based IPsec VPN** | `security policies ... then permit tunnel ipsec-vpn <vpn>` | Bind an IPsec tunnel to a security policy (traffic matched by the policy enters the tunnel) instead of routing it out a route-based st0/XFRM interface. A top-5 vSRX idiom operators reach for immediately. | Medium | **Missing — the `then permit tunnel ipsec-vpn` policy action is unsupported.** #3114 rejects only that action because the compiler would otherwise silently drop its tunnel modifier and turn it into unconditional permit (fail-open). This action limitation is not a rejection of an IPsec VPN object; route-based st0/XFRM VPNs remain supported. Route traffic to an st0 unit / XFRM interface bound to its VPN instead. |
...
| **IPsec SA Synchronization** | `chassis cluster ... ipsec-session-synchronization` | Sync IPsec Security Associations between nodes. Avoids tunnel re-establishment after failover. | Medium | Partial: SAs are not synchronized. The primary sends active connection names every 30s, and a new primary re-initiates each one via `swanctl --initiate`, so every RG failover, planned or not, has a per-tunnel IKE renegotiation window and depends on the remote peer accepting a fresh IKE (#9415) |
...
| **Three-Color Policer** | `firewall three-color-policer ...` | RFC 2697/2698 metering with green/yellow/red marking based on CIR/CBS/EBS or CIR/PIR | Medium | Legacy eBPF/~~DPDK~~ (DPDK retired #1525) done; userspace AF_XDP supports color-blind `then discard` with compatible snapshot continuity. A color-blind `then loss-priority <level>` is METER-ONLY (#9503): the policer counts green/yellow/red and forwards the excess unmarked, and the commit warns. Before #9503 it disarmed forwarding on every binding, and the helper would have failed the referencing terms closed. A policer with no explicit color statement defaults to color-blind (Junos parity, #4535) and is enforced rather than disarming forwarding. #1375 is closed; remaining color-aware/non-drop action parity plus integration, failover, and performance hardening are production/future parity work, not active #1373 source-removal blockers. |
...
| **RADIUS Authentication** | `system radius-server ...; system authentication-order radius` | External RADIUS authentication for management access (SSH, CLI, web) | Medium | Refused at strict commit (#10831): local-only authentication only. |
| **TACACS+ Authentication** | `system tacplus-server ...; system authentication-order tacplus` | External TACACS+ authentication with per-command authorization | Medium | Refused at strict commit (#10831): local-only authentication only. |
| **SNMP v3 USM** | `snmp v3 usm local-engine user ...` | Full SNMPv3 with authentication (SHA/MD5) and privacy (AES/DES). xpf parses v3 users but runtime may be incomplete. | Medium | Partial (parsed, needs runtime verification). `vacm`, `notify` / `target-*`, `usm remote-engine` and `engine-id` are accepted with a commit advisory and NOT implemented (#9414) |
...
| **RADIUS Server Config** | `system radius-server ... port ... secret ...` | RADIUS server definitions for AAA (authentication, authorization, accounting) | Medium | Refused at strict commit (#10831): local-only authentication only. |
| **TACACS+ Server Config** | `system tacplus-server ... port ... secret ...` | TACACS+ server definitions for per-command authorization | Medium | Refused at strict commit (#10831): local-only authentication only. |
| **Authentication Order** | `system authentication-order [radius tacplus password]` | Control order of authentication methods for management access | Medium | Refused at strict commit (#10831): local-only authentication only. |
...
| **DHCPv6 Relay** | `forwarding-options dhcp-relay dhcpv6 ...` | RFC 8415 Relay-Forw / Relay-Reply relay agent with multicast client intake, configured UDP/547 upstream forwarding, Interface-ID inheritance, HA gating, and startup/session recovery | Medium | Implemented (#9553) — the supported DHCPv6 stanza compiles to a distinct typed relay and runs in `pkg/dhcprelay`; unsupported or incomplete children remain a scoped strict refusal/tolerant warning rather than a DHCPv4 fallback. See [the #9411 history](config-schema.md#forwarding-options-dhcp-relay-dhcpv6--supported-rfc-8415-subset-9553-historical-refusal-9411). |
...
| **VPLS** | `routing-instances ... instance-type vpls` | Virtual Private LAN Service for L2 VPN | Low | Missing (rejected at strict commit since #9814 — `instance-type` accepts only `forwarding`, `virtual-router`, `vrf`) |
