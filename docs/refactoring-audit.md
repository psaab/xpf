...
(`make audit-refresh`), NOT by PR authors — see
[The three gates](#the-three-gates). What a PR author is gated on is growing a file
...
### 2. The accepted-entry liveness gate (hard)

`TestAcceptedEntriesAreLive` is tree-global rather than diff-local. It reads
all accepted paths from the working tree, requires each path to exist, and
fails when raw LOC is below the entry's tier floor. It runs even when the
changed set is empty. The check assumes a full checkout; sparse or partial
checkouts fail closed when an accepted path is absent locally.

### 3. The freshness job (advisory)
