...
Filters: `protocol tcp|udp|icmp|icmpv6|gre|esp|ipip|ipv6|ospf|egp|igmp|pim|ah|sctp|vrrp`,
`icmp6`, and aliases `junos-tcp-any|junos-udp-any|junos-icmp-all|junos-ping|junos-icmp6-all|junos-pingv6|junos-gre|junos-ospf|junos-ip-in-ip|junos-ipip`, or canonical numeric `0..255` (case-insensitive names; bare digits only for numerics, so `007` is valid while `+6` and space-padded `6` are not), `source-prefix X.X.X.X/N`, `destination-prefix X.X.X.X/N`,
...
- Direct gRPC `GetSessions` and `ClearSessions`
  ...
  `sctp` and `ospf`) still match their sessions correctly.
  `ProtocolNumberLenient` equals the strict `ProtocolNumber` resolver for the
  current tables (`#3393` closed the one-way gap for `ipv6` = 41) and remains
  the stable filter seam for any future render-only name. The shared parser
  accepts display-only names such as `ipv6`, numeric values `0..255`, and
  surrounding whitespace on names, but rejects signed or whitespace-padded
  numerics and out-of-range values. The strict `appid.ProtocolNumber` SSOT
  used by config compilation and policy matching is unchanged.
...
- **`junos-host` self-traffic policy (#3019):** a `from-zone <z> to-zone
  ...
  - **Packets-dropped / named pre-L3 accounting (#4477/#10498):** the
    ...
    flow statistics` are fed by the userspace counter bridge. The Go manager
    sums each BindingStatus reason across bindings and pushes per-poll deltas
    into the dedicated global indices. `GlobalCtrDrops` folds policy denies,
    screen/IDS drops, host-inbound denies, NAT allocation failures, unknown
    VLAN admission drops, and destination-MAC admission drops, so the named
    rows are a truthful breakdown of that aggregate. REST (`unknown_vlan_drops`
  ...
  - **`Packets dropped` scope — configured enforcement/admission only
    ...
    - **UMEM-slice hygiene drops** (#10498): an invalid descriptor range is
      ...
      helper status and `xpf_userspace_umem_slice_dropped_total`. It is not a
      configured enforcement decision and is intentionally excluded from both
      `GlobalCtrDrops` and the global reason-index family.
    - **No-route** (userspace has no route to the destination) and
      **missing-neighbor** (route resolved but ARP/ND unresolved) drops are
      ...
      - **Martian drops** (#4743): a no-route drop whose destination is a
        martian address is also counted distinctly as `martian_dropped` and
        shown as `Martian drops:`. It is a strict sub-breakout of
        `route_miss_packets`.
    - **IPv6 extension-header fail-closed drops** (#4743): an over-limit,
      uninspectable extension-header chain is counted as
      `ipv6_ext_header_dropped`, shown as `IPv6 ext-header drops:`, and is not
      in `Packets dropped`.
    ...
    - **NAT64 fail-closed drops** — distinct from source-NAT allocation
      failure, which is in the total.
