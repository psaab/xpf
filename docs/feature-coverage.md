...
- **DHCP relay**: DHCPv4 and the supported RFC 8415 DHCPv6 Relay-Forw /
  Relay-Reply agent. DHCPv6 uses `forwarding-options dhcp-relay dhcpv6` with
  multicast client intake, configured UDP/547 upstream servers, per-group
  Interface-ID inheritance, and the existing HA master gate (#9553). The
  unsupported-child remainder is still refused strictly and warned
  tolerantly; see the #9411 history in `docs/config-schema.md`.
...
| VLANs (802.1Q) | Yes. The XDP dataplane derives VLAN (security-domain) identity SOLELY from the in-frame tag; a parent NIC's RX-VLAN hardware offload (which strips the tag into `skb->vlan_tci`, unreadable by XDP) MUST be disabled. **#5268 (security): disabling it is an ACTIVATION PRECONDITION** — if `ensureRxVlanOff` cannot turn the offload off AND the parent carries ≥1 configured VLAN subinterface, compile/apply FAILS CLOSED rather than attaching the shim; otherwise HW-stripped tagged frames parse as `vlan_id=0`, fall back to the parent ifindex, and are classified into the parent's/first-subinterface's zone — a cross-zone screen/policy bypass. A NIC that reports the feature `off`/`off [fixed]` (e.g. virtio) or ABSENT never strips tags and is unaffected, as is a plain parent with no 802.1Q units. (#5268 also corrected a latent `strings.Contains(line,"off")` bug that matched the feature name `rx-vlan-offLOAD` and so read an ACTIVE offload as already-off — the offload is now detected by the value after the colon.) **#10655 (802.1ad S-tag):** a single outer 0x88a8 S-tag on an XDP-adjudicated ingress is dropped and counted as `stag_drop`, never steered as a same-VID 802.1Q unit; double-tagged frames retain `qinq_drop` (#9888). The separate S-tag RX-strip offload can hide the tag before XDP and remains a hardening follow-up (#10915). |
