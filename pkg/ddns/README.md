...
- **A TTL-only change is a change (#9067)** — Surface A change detection was
  `changed := addr != rt.lastAddr`, so a TTL edit was not a change on ANY
  backend and nothing republished until the forced-refresh floor (24h). That
  is a *delay* everywhere; on Cloudflare the backend's content-only early
  return made it *permanent*. Both defeat the same operator intent: a shorter
  TTL is bought precisely so a change propagates soon, and waiting a day for
  the TTL itself to land inverts that. `surfaceAState.lastTTL` now records the
  TTL of the last successful publish and a delta marks the scope changed.
  Two details are load-bearing. The comparison runs on the **normalised** TTL
  (`normalizeDDNSTTL`, shared with the publish path) because `TTL <= 0` means
  "use `defaultDDNSTTL`" — comparing the raw field would both miss an
  explicit→unset edit and invent a change on unset→explicit-default. And the
  stored value is the normalised one, because `lastTTL == 0` means "not
  published in this process" and suppresses the comparison (so a scope
  re-adopted after restart, whose TTL the durable store does not record, is not
  republished spuriously) — storing a raw 0 for a scope published with the
  leaf unset would be indistinguishable from "never published" and would
  disable the comparison for that scope permanently.
