...
literal classification (Rust parity + the FRR `X:X::X:X` gateway slot),
and is refused outright as a backup-router next-hop instead (#9820,
which deleted `frrOperandIsV6` and its mapped-⇒-v4 rule).
...
**The bounded streaming read has two independently-admitted callers (#9755,
#10708).** `StreamBGPRoutes` calls the executor's `VtyshStream` directly: it
takes no `VtyshLimiter` slot and no 15s `vtyshTimeout`. The REST caller holds
`ribStreamLimiter` (capacity 2) and a 10-minute total budget. The gRPC unary
caller holds `grpcBGPStreamLimiter` (capacity 2) and a 10-minute scan budget;
its rendered result is additionally capped at 100000 routes and 8 MiB. The two
admission gates are separate, so worst case across the process is four buffered
plus four streaming `vtysh` children. The streaming caller census pins both
limits and budgets in `TestStreamPathKeepsEveryCallerBounded9755`.
