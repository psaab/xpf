...
func TestEncodeDeleteScopedV4RoundTrip10512(t *testing.T) {
	...
	k, gen, fwd, domain, id, ok := parseDeleteV4Wire(payload)
	if !ok {
		t.Fatal("scoped v4 payload does not decode")
	}
...
func TestEncodeDeleteScopedV6RoundTrip10512(t *testing.T) {
	...
	k, gen, fwd, domain, id, ok := parseDeleteV6Wire(payload)
	if !ok {
		t.Fatal("scoped v6 payload does not decode")
	}
...
func TestEncodeDeleteBareLayoutFrozen10512(t *testing.T) {
	...
	_, _, _, domain, id, ok := parseDeleteV4Wire(v4)
	if !ok || domain != 0 || id != 0 {
		t.Fatalf("bare v4 must decode with zero scope, got domain=%d id=%d ok=%v", domain, id, ok)
	}
	v6 := encodeDeleteV6(scopedCodecKeyV6(), 7, false)[syncHeaderSize:]
	if len(v6) != 49 {
		t.Fatalf("bare v6 payload len = %d, want 49", len(v6))
	}
	_, _, _, domain6, id6, ok6 := parseDeleteV6Wire(v6)
	if !ok6 || domain6 != 0 || id6 != 0 {
		t.Fatalf("bare v6 must decode with zero scope, got domain=%d id=%d ok=%v", domain6, id6, ok6)
	}
}
...
func TestParseDeletePartialTailRejected10512(t *testing.T) {
	...
	for _, n := range []int{26, 29, 33, 36} {
		_, _, _, _, _, ok := parseDeleteV4Wire(full[:n])
		if ok {
	...
	for _, n := range []int{50, 53, 57, 60} {
		_, _, _, _, _, ok := parseDeleteV6Wire(full6[:n])
		if ok {
...
func TestScopedDeleteReadableByOldDecoder10512(t *testing.T) {
	...
	k, gen, fwd, _, _, ok := parseDeleteV4Wire(scoped[:25])
	if !ok {
		t.Fatal("old-length prefix does not decode")
	}
	...
	k6, gen6, fwd6, _, _, ok6 := parseDeleteV6Wire(scoped6[:49])
	if !ok6 {
		t.Fatal("old-length v6 prefix does not decode")
	}
...
func TestEncodeDeleteScopedByIDOnlyCarriesTail10512(t *testing.T) {
	...
	_, _, _, domain, id, ok := parseDeleteV4Wire(msg)
	if !ok || domain != 0 || id != 0xABCDEF {
		t.Fatalf("id-only tail must round-trip, got domain=%d id=%#x ok=%v", domain, id, ok)
	}
}
