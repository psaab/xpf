...
type compileOpts struct {
	...
	// lenientReservedRoutingInstanceName (#9622) skips the reserved
	// routing-instance name gate (validateReservedRoutingInstanceNamesAST).
	// Strict commit rejects an instance named for a daemon-reserved VRF (the
	// management VRF). The tolerant load / peer-sync paths skip the gate so an
	// already-persisted config still BOOTS (#1960 no-brick), and
	...
	// lenientAddressUnimplementedForms (#9524) downgrades the mixed address
	// value-form gate (validateAddressUnimplementedFormsStrict) to a warning on the
	...
	// lenientZoneGroupExpansion9656 (#9656) downgrades the refusal of a
	// security-zone group that the fan-out budget left unexpanded to a warning
	// on the tolerant load and peer-sync paths (#1960).
	lenientZoneGroupExpansion9656 bool
...
func lenientCompileOpts() compileOpts {
	return compileOpts{
		...
		lenientZoneGroupExpansion9656:          true,
