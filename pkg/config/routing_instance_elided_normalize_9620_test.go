...
var elidedInstanceCells9620 = []struct {
	...
	{"bracketed interface list",
		...
		`ri1 { interface [ ge-0/0/1.0 ge-0/0/2.0 ]; instance-type vrf; }`},
	{"apply-macro with a braced body",
		`ri1 apply-macro M { interface ge-0/0/1.0; }`,
		`ri1 { apply-macro M { interface ge-0/0/1.0; } }`},
	{"apply-macro arguments spelled as keywords",
		`ri1 instance-type vrf apply-macro M interface ge-0/0/1.0;`,
		`ri1 { instance-type vrf; apply-macro M interface ge-0/0/1.0; }`},
