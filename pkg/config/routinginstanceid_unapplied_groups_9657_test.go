...
func TestRoutingInstanceNameScanSkipsApplyStatements_9657(t *testing.T) {
	...
	names := routingInstanceNameUnionAST(tree, nil)
...
// CompileConfigForNode accepts any integer node ID, negative ones included, and
// expands that node's own groups (`node%d`). node0's and node1's views never see
// `groups node2` or `groups node-1`, so the gates must count the requested
// node's view: a collision and a reserved instance that only that node's
// expansion lands must still be refused on its strict compile.
func TestGatesCountTheRequestedNodesView_9657(t *testing.T) {
	...
	for _, id := range []int{2, -1} {
		...
		collide := groups(braced116_9657) + activeRI7_9657
		if _, err := CompileConfigForNode(parse(collide), id); err == nil || !strings.Contains(err.Error(), "table-id collision") {
			t.Errorf("#9657: node %d's own expansion lands ri116 beside ri7, so its strict compile must refuse the collision, got %v", id, err)
		}
		cfg, err := CompileConfigForNodeLenient(parse(collide), id)
		if err != nil {
			t.Fatalf("lenient node %d compile: %v", id, err)
		}
		quarantined := false
		for _, w := range cfg.Warnings {
			if strings.Contains(w, "QUARANTINED") {
		...
		if !quarantined {
			t.Errorf("control: node %d's expansion must land both instances and quarantine one; instances=%v", id, riNames9657(cfg))
		}
		reserved := groups("routing-instances {\n            mgmt {\n                instance-type virtual-router;\n            }\n        }\n")
		if _, err := CompileConfigForNode(parse(reserved), id); err == nil || !strings.Contains(err.Error(), "reserved for") {
			t.Errorf("#9657: a reserved mgmt instance in node %d's group must be refused on its strict compile, got %v", id, err)
		}
	}
}
