...
// The brace-elision pass's SCOPE TABLE, split from compact_normalize_8662.go
// (#8954). The table and the fold mechanics have different growth rates: this
// file gains a line every time a pair is adjudicated -- which during the #8879
// / #8943 campaign was most hours -- while the mechanics change rarely and for
// unrelated reasons. Keeping them together pushed the combined file to the
// 2000-LOC [REFACTOR] floor and made every admission diff touch the algorithm.
//
// Nothing here changed in the split: the function is moved verbatim, and an
// admission is still governed by the criteria in its doc comment below.
