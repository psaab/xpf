...
func TestShowTextLLDPNeighborsQuotesAndBoundsPeerFields10899(t *testing.T) {
	seen := time.Now()
	...
	s := NewServer("", Config{
		...
		LLDPNeighborsFn: func() []*lldp.Neighbor { return neighbors },
	})
	resp, err := s.ShowText(context.Background(), &pb.ShowTextRequest{Topic: "lldp-neighbors"})
	if err != nil {
		t.Fatalf("ShowText(lldp-neighbors) error = %v", err)
	}
	out := resp.GetOutput()
