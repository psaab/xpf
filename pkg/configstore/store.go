...
type Store struct {
	...
	// confirmArmDegraded records that finalizing or restoring a durable
	// commit-confirmed record failed after the candidate became active. A
	// first-arm record is written before active.json, so its write failure
	// rejects the commit and never creates this debt.
	//
	// confirmArmGen pins WHICH live window the retry belongs to. Re-driving
	// after that window is resolved or replaced must not resurrect its
	// rollback record.
