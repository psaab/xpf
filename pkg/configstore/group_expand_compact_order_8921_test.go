...
// `hold-time 9` directly after a packed inline neighbor -- a node the compiler
// never sees -- made commit-check decline the fold and the override failed.
//
// PARITY IS ASSERTED, not claimed: every override fixture must be accepted by
// commit-check AND by the strict compiler. The fixtures carry a group-level
// peer-as for that reason; without it the compiler refuses the neighbor for an
// unrelated missing peer-as, and a commit-check-only cell would stay green
...
func TestCommitValidationNormalizesBeforeGroupExpansion8921(t *testing.T) {
	...
	const bracedInline = `protocols { bgp { group g1 { peer-as 65001; neighbor 192.0.2.1 { hold-time 30; } } } }`
	// The control's inline side supplies the peer-as and nothing that
	// overrides the inherited hold-time.
	const controlInline = `protocols { bgp { group g1 { peer-as 65001; } } }`
	cases := []struct {
		...
		{"braced group body, compact inline override beside an inactive sibling", bracedGroup,
			`protocols { bgp { group g1 { peer-as 65001; neighbor 192.0.2.1 hold-time 30; inactive: hold-time 9; } } }`},
	}
	...
	for _, c := range cases {
		for _, nodeID := range []int{-1, 0} {
			t.Run(c.name, func(t *testing.T) {
				...
				err := schemaValidateExpandedTreeForNode(parse(t, pre+c.group+controlInline), nodeID)
				...
				text := pre + c.group + c.inline
				if err := schemaValidateExpandedTreeForNode(parse(t, text), nodeID); err != nil {
					t.Errorf("node %d: commit-check refused the override: the inline hold-time 30 "+
						"must override the group's 2, as it does in the compiler: %v", nodeID, err)
				}
				if _, err := config.CompileConfig(parse(t, text)); err != nil {
					t.Errorf("PARITY: the strict compiler refuses this fixture (%v), so an "+
						"accepting commit-check would not be agreeing with it about anything", err)
				}
			})
		}
	}
