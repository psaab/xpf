...
type Manager struct {
	...
	loaded atomic.Bool
	// attachedLinksObserver is the #9725 attached-link observer the daemon's
...
func (m *Manager) Close() error {
	...
	// #9725: report the links that OUTLIVE this Close -- the ones whose pin
	// still names them. Computed before the handles close, because afterwards
	// none of them can answer. Teardown calls closeLocked directly instead, so
	// its own report of zero is the ONLY one that reaches the gate: reporting
	// survivors after it would reopen transit in the middle of a teardown that
	// is about to destroy those very pins.
	m.notifyAttachedLinksFunc(m.survivingPinnedXDPLinkCount)
	return m.closeReportless()
}

// closeReportless is Close without the attached-link report, for callers that
// have already reported the count that will hold when they finish.
func (m *Manager) closeReportless() error {
...
func (m *Manager) Teardown() error {
	// #9725: Teardown destroys the pinned kernel objects, so nothing survives it.
	// Report zero BEFORE the close, and use the reportless close so Close cannot
	// then report the survivors it still sees and reopen the gate mid-teardown.
	m.notifyAttachedLinks(0)
	m.closeReportless()
