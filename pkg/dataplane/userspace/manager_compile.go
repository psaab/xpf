...
func (m *Manager) syncInterfaceAttachments(result *dataplane.CompileResult, snapshot *ConfigSnapshot) {
	...
	// #9725: no gate call here. DetachXDP reports the count it LEAVES before it
	// unpins and closes, so this reconcile closing the last link drives the gate
	// without knowing the gate exists — including when Close returns an error,
	// which removes the map entry anyway.
