...
// #10689 rejects martian sources on transit before policy, session or egress
...
/// `frag_v4_transit_meta` with the source zeroed for the NoRoute control below.
///
/// The parsed source remains an unspecified address, but the source-class gate
/// intentionally excludes `NoRoute`, so policy still adjudicates this packet.
