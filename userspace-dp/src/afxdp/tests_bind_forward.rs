...
// ---------------------------------------------------------------------------
// #7174 C19, SECOND REACHABILITY PATH — measured, and the answer is that the
// merged C19 gate does NOT cover this arm.
//
// The merged fix keys on `extra.is_fragment && !extra.l4_present`. The cached
// FLOWLESS arm of `resolve_cached_cos_tx_selection_impl` applies
// `TermMatchExtra::default()`, so `is_fragment` is FALSE there and the gate
// cannot fire. Confirmed at master by running this measurement: ARM A
// (flow-keyed) answers drop=false, ARM B (flowless, the fragment case) answers
// drop=true, for the same packet and the same filter.
//
// WHY THE ARM'S OWN SAFETY ARGUMENT MISSES IT. That arm's #6055 comment says
// ports are forced to 0 and therefore "a port-BEARING term never matches (no
// spurious drop)". That is true of `from destination-port 22` — 0 is not 22, so
// it does not match. It is FALSE of `destination-port-except 22`, which matches
// everything that is NOT 22, and literal 0 is not 22 — so the exception matches
// and the discard term fires on a packet whose real port was excepted. The
...
// `l4_present: false` and it red 11+ tests
// (`a_cached_flow_still_matches_a_port_term_despite_no_l4_present_7174` is the
// guard for that). The literal `0, 0` at this call site is what needs
// addressing.
//
