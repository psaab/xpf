...
pub(crate) struct WgCounters {
    ...
    /// #9594/#10527: authenticated transport records for a STEERED listen port
    /// that reached its control thread through the kernel and whose inner packet
    /// was TRANSIT. Dropped instead of written to the wgN TUN, where the kernel
    /// would have forwarded it with no zone policy. Covered-ingress records
    /// occur while the dataplane is degraded; #10527 applies the same refusal
    /// to shim-uncovered configured ingresses. Inner packets addressed to the
    /// firewall itself are still delivered.
