...
// #9902 review: `match_source_nat` is #[cfg(test)]-gated (Untracked
// allocator) — this import exists only for the test-only
// `match_source_nat_for_flow` helper below it in `forwarding/nat.rs`.
#[cfg(test)]
use crate::nat::match_source_nat;
...
use neighbor_dispatch::{
    ...
    learn_dynamic_neighbor_after_admission, learn_dynamic_neighbor_from_packet,
    pending_neigh_admission, pending_neigh_flow_key, record_pending_neigh_admission_drop,
    retry_pending_neigh,
};
