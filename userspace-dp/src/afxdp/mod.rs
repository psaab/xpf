...
// doesn't drop frames. PendingNeighPacket is 232 B on x86_64 (XdpDesc +
// UserspaceDpMeta + SessionDecision + boxed flow key + queued_ns +
// probe_attempts), so worst-case per binding when the queue is fully
// populated is ~928 KiB plus one boxed SessionKey per queued packet.
// To avoid paying that ~928 KiB up front per binding × N bindings, the
