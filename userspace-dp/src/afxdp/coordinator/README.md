...
| `bpf_maps.rs` | `BpfMaps` — pinned BPF map FDs (XSK map, heartbeat, session, conntrack v4/v6) opened once and shared with every worker. Also carries the steering map's row-owner registry (`session_map_owners`, #9560): one per `BpfMaps`, so each bringup, and `stop`, starts it empty. |
