...
/// bumped in that case (no double-drop / double-count). Since #10705 the
/// same pre-build decision converts the DF-clear oversize inner into a PTB
/// as well (forward-whole is not an option on a tunnel path — this guard
/// would refuse the frame), so this drop+bump now fires only when the PTB
/// itself was suppressed or unbuildable, or the inner MTU was unknown
/// (fail-open forward): the residual backstop, not the common case.
...
pub(super) fn encapsulate_native_gre_frame(
    ...
    // build whenever a PTB is emitted (#10705 extended that to DF-clear
    // inners). What reaches this drop is the residual backstop: a
    // suppressed/unbuildable PTB or an unknown inner MTU (fail-open).
